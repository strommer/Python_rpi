...
class LeafNode(object):
    ...
    def search_key_full_eq (self, key) : 
        ...
        priorNodesVisited = nodes_visited
        found=[]
        while(self.sibling!=None): 
            nodes_visited+=1;       
            tempKey=[]
            row=[]
            for i in xrange(len(self.tuples)) :
                for j in xrange(len(self.tuples[i].val())) :  
                       if j!=nameFlag and j!=showidFlag and j!=finalscoreFlag and j!=placeFlag:
                           row.append(self.tuples[i].val()[j])   
                tempKey.append(row)         
                row=[]    
            for i in xrange( len(self.tuples) ):
                   if tempKey[i] == key:
        ...
        print "Visited "+str(nodes_visited-priorNodesVisited+1)+" leaf nodes "
        return found
    def __str__ (self) :
        tuple_str = ""
...
class InternalNode(object):
    ...
    def search_key_full_eq(self, key):
        ...
        for i in xrange(len(self.keys)) :
            if nameFlag==0 :
                print "   visited internal ==>", str(self.keys[0]) + " go to left"
                return self.pointers[0]    
            if self.keys[i] > key:
                if i > 0:
                    print "   visited internal ==>", str(self.keys[i]) + " go to left"
                    return self.pointers[i]
                else:
                    print "   visited internal ==>", str(self.keys[0]) + " go to right"
                    return self.pointers[0]
...
def search ( root, key ):
    ...
    finalscoreFlag = -1
    placeFlag = -1
    print "Searching i1 for equality (%s,%s,%s,%s)" %(key[0], key[1], key[2], key[3])
		 
    if key[0] == "*" : nameFlag=0
    if key[1] == "*" : showidFlag=1
    if key[2] == "*" : finalscoreFlag = 2
    if key[3] == "*" : placeFlag = 3
    
    for i in xrange(len(key)) :
        if key[i] != "*" :   tempKeyToBeSearched.append(key[i])

	
    cur_node = root
    nodes_visited = 1
    while not cur_node.isleaf():
        cur_node = cur_node.search_key_full_eq( tempKeyToBeSearched )
        nodes_visited += 1
    found = cur_node.search_key_full_eq( tempKeyToBeSearched )
    ...
    if len(found) == 0:
        ...
        print "\n"
        print str(len(found))+" Records found."
...
if __name__ == "__main__":
    ...
    print "Search results for Sameer Sawla (sawlas)"
    cmd_file = sys.argv[1]
