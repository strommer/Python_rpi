...
class Interval:
    ...
    def __repr__(self):
        if self.error == 0:
            return "%s%s%g%s"%(self.__class__.__name__,
                    "(value=",self.value,')')
        return "%s%s%g,%s%g%s"%(self.__class__.__name__,
    ...
    @property
    def max(self):
        return (2*self.value - self.min)

    def __mul__(self,other):
        if type(other) is int:
            other = Interval(value=other,error=0)
        result_min = min(self.min*other.min,
                         self.min*other.max,
                         self.max*other.min,
                         self.max*other.max)
        result_max = max(self.min*other.min,
                         self.min*other.max,
                         self.max*other.min,
                         self.max*other.max)

        return Interval(value=None,error=None,
                        min=result_min,max=result_max)
    
    def __rmul__(self,other):
        if type(self) is int:
            self = Interval(value=other,error=0)
        return self * other

    def __add__(self,other):
       
        result_min = self.min + other.min
        result_max = self.max + other.max
        return Interval(value=None,error=None,
                        min=result_min,max=result_max)
    def __sub__(self,other):
    
        result_min = self.min - other.max
        result_max = self.max - other.min
        return Interval(value=None,error=None,
                        min=result_min,max=result_max)
